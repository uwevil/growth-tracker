...
type ChartKey = 'height' | 'weight' | 'head';

// ─── State ────────────────────────────────────────────────────────────────────
let patients: Patient[] = [];
let selectedPatientId: string | null = null;
let editingMeasurementId: string | null = null;
const charts: Record<ChartKey, any> = { height: null, weight: null, head: null };

const API = '/api';

// ─── Helpers ──────────────────────────────────────────────────────────────────
function selectedPatient(): Patient | undefined {
...
function ageLabel(birthDateIso: string, measureDateIso: string): string {
    if (!birthDateIso || !measureDateIso) return '';
...
function currentAge(birthDateIso: string): string {
...
async function apiCreatePatient(data: { name: string; birthDate: string; gender?: string }): Promise<Patient> {
    const res = await fetch(`${API}/patients`, {
        method: 'POST',
...
async function apiUpdatePatient(id: string, data: { name: string; birthDate: string; gender?: string }): Promise<Patient> {
    const res = await fetch(`${API}/patients/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(data)
    });
    if (!res.ok) throw new Error('Erreur mise à jour patient');
    return res.json();
}
...
async function apiCreateMeasurement(patientId: string, data: Omit<Measurement, 'id'>): Promise<Measurement> {
    const res = await fetch(`${API}/patients/${patientId}/measurements`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(data)
    });
    if (!res.ok) throw new Error('Erreur création mesure');
    return res.json();
}

async function apiUpdateMeasurement(patientId: string, id: string, data: Partial<Measurement>): Promise<Measurement> {
    const res = await fetch(`${API}/patients/${patientId}/measurements/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(data)
    });
    if (!res.ok) throw new Error('Erreur mise à jour mesure');
    return res.json();
}

async function apiDeleteMeasurement(patientId: string, id: string): Promise<void> {
    const res = await fetch(`${API}/patients/${patientId}/measurements/${id}`, { method: 'DELETE' });
    if (!res.ok) throw new Error('Erreur suppression mesure');
}

// ─── Patient UI ───────────────────────────────────────────────────────────────
function renderPatientsList(): void {
...
function selectPatient(id: string): void {
    if (selectedPatientId === id) return;
...
function renderPatientContent(): void {
...
// ─── Charts ───────────────────────────────────────────────────────────────────
function buildChartConfig(label: string, unit: string, color: string, dataPoints: { x: string; y: number }[]): object {
    return {
        type: 'line',
        data: {
            datasets: [{
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: {
                    ...
                    bodyFont: { size: 12 },
                    padding: 10,
                    callbacks: { label: (ctx: any) => ` ${ctx.parsed.y} ${unit}` }
                },
                zoom: {
...
function resetZoom(key: ChartKey): void {
    if (charts[key]) charts[key].resetZoom();
}
(window as any).resetZoom = resetZoom;

function updateOrCreateChart(key: ChartKey, label: string, unit: string, color: string, data: { x: string; y: number }[]): void {
    const canvasId = `chart-${key}`;
    const container = document.getElementById(canvasId)!.parentElement!;

    if (data.length === 0) {
        let empty = container.querySelector('.chart-empty') as HTMLElement | null;
        if (!empty) {
...
function renderCharts(p: Patient): void {
    const h = p.measurements.filter(m => m.height !== undefined).map(m => ({ x: m.date, y: m.height as number }));
    const w = p.measurements.filter(m => m.weight !== undefined).map(m => ({ x: m.date, y: m.weight as number }));
    const hc = p.measurements.filter(m => m.headCircumference !== undefined).map(m => ({ x: m.date, y: m.headCircumference as number }));
...
// ─── Table ────────────────────────────────────────────────────────────────────
function renderTable(p: Patient): void {
...
// ─── Measurement form ─────────────────────────────────────────────────────────
function resetMeasurementForm(): void {
    editingMeasurementId = null;
    ($('measurement-form') as HTMLFormElement).reset();
    $('edit-id').textContent = '';
...
function editMeasurement(id: string): void {
    const p = selectedPatient();
    if (!p) return;
    ...
    ($('measure-notes') as HTMLTextAreaElement).value = m.notes ?? '';
...
async function confirmDelete(id: string, dateLabel: string): Promise<void> {
    const p = selectedPatient();
    if (!p) return;
...
(window as any).selectPatient = selectPatient;
(window as any).openEditPatient = openEditPatient;
(window as any).deletePatient = deletePatient;
(window as any).editMeasurement = editMeasurement;
...
function setupEventListeners(): void {
    ...
    $('patient-form').addEventListener('submit', async (e) => {
        ...
        const gender = ($('patient-gender') as HTMLSelectElement).value as 'male' | 'female' | '';
        ...
        try {
            if (editId) {
                ...
                if (idx !== -1) patients[idx] = { ...patients[idx], ...updated };
                if (selectedPatientId === editId) renderHeaderInfo();
    ...
    $('measurement-form').addEventListener('submit', async (e) => {
        ...
        const notesVal = ($('measure-notes') as HTMLTextAreaElement).value.trim();
        ...
        try {
            if (editingMeasurementId) {
                ...
                if (idx !== -1) p.measurements[idx] = updated;
